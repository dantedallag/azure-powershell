...
function Remove-AzSubscriptionDeploymentStackCustom {
    [OutputType([bool])]
    [CmdletBinding(DefaultParameterSetName = 'RemoveByName', PositionalBinding = $true, SupportsShouldProcess, ConfirmImpact = 'High')]
    ...
    param(
        [Parameter(Position = 0, Mandatory, ValueFromPipelineByPropertyName, ParameterSetName = 'RemoveByName', HelpMessage = 'The name of the stack.')]
        [Alias('StackName')]
        [string]
        ${Name},
    ...
    process {
        ...
        if ($PSBoundParameters.ContainsKey("StackObject")) {
            $PSBoundParameters["Name"] = $PSBoundParameters["StackObject"].Name
