...
# function Set-AzSubscriptionDeploymentStackCustom {
...
#     [Microsoft.Azure.PowerShell.Cmdlets.Resources.DeploymentStacks.Description('Updates a subscription scoped deployment stack')]
#     param(
...
#             $warningMessage = "No existing deployment stack found. Create new Subscription scoped DeploymentStack in current Subscription with the following actions:" 
#             $warning = StackExistWarning $warningMessage $shouldDeleteResources $shouldDeleteResourceGroups
            
#             if (($null -eq $currentStack) -or $PSBoundParameters.ContainsKey('Force') -or ($PsCmdlet.ShouldProcess($PSBoundParameters["Name"], $warning))) {
#                 Az.DeploymentStacks.internal\Set-AzDeploymentStacksDeploymentStack @PSBoundParameters
#             }
#         }
#         catch {
#             throw
