...
namespace Microsoft.Azure.Commands.ResourceManager.Cmdlets.Implementation.CmdletBase
{
    ...
    public abstract partial class DeploymentCreateCmdlet : DeploymentWhatIfCmdlet
    {
        ...
        protected override void OnProcessRecord()
        {
            ...
            {
                var whatIfInitialResult = this.ExecuteWhatIf();

                // -----------------------------------------------------------------------------------------------------------------------------------------------

                // Only run POC code if what if succeeded
                if (whatIfInitialResult.Error == null && whatIfInitialResult.Status == "Succeeded")
                {
                    var whatIfOperationResult = whatIfInitialResult.whatIfOperationResult;

                    if (Poc1WhatIf.IsPresent)
                    {
                        Poc1(whatIfOperationResult);
                    }
                    else if (Poc2WhatIf.IsPresent)
                    {
                        Poc2(whatIfOperationResult);
                    }
                    else if (Poc3WhatIf.IsPresent)
                    {
                        Poc3(whatIfOperationResult);
                    }

                    whatIfResult = new PSWhatIfOperationResult(whatIfOperationResult);
                }
